...
import { makeMerkleTreeAPI, merkleTreeObj } from './generated_keys.js';
import { pubkeys } from './keyring.js';
...
test.before(async t => {
  const { accounts: testAccounts } = merkleTreeObj;
  const getProp = prop => obj => prop[obj];

  const setup = await commonSetup(t);

  await setup.setupSpecificKeys(testAccounts.map(x => x.mnemonic));
...
const makeDoOfferHandler = async (
  ...
  const offerArgs = {
    proof: makeMerkleTreeAPI(pubkeys, accounts).constructProof(
  ...
  console.log('----------------------------------------');
  console.log('proof ::::', offerArgs.proof);
  console.log('----------------------------------------');
...
// test.serial('makeClaimTokensInvitation offer :: 1txn/min', async t => {
//   const { provisionSmartWallet } = t.context.vstorageClient;
//   provisionSmartWallets(accounts.slice(0, 1));
// });
// test.serial('makeClaimTokensInvitation offer :: 2txn/min', async t => {
//   const { provisionSmartWallet } = t.context.vstorageClient;
//   provisionSmartWallets(accounts.slice(1, 3));
// });
// test.serial('makeClaimTokensInvitation offer :: 3txn/min', async t => {
//   const { provisionSmartWallet } = t.context.vstorageClient;
//   provisionSmartWallets(accounts.slice(3, 1));
// });

// test.serial('makeClaimTokensInvitation offer :: 4txn/min', async t => {
//   const { provisionSmartWallet } = t.context.vstorageClient;
//   provisionSmartWallets(accounts.slice(0, 1));
// });
