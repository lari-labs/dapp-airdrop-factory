...
import { OPEN, PREPARED } from '../../src/airdrop/airdropKitCreator.js';
...
const startLocalInstance = async (t, bundle, { issuers: { Fee }, zoe }) => {
  const {
    powers: mockPowers,
    vatAdminState,
    chainStorage,
  } = await mockBootstrapPowers(t.log);

  const chainTimerService = await mockPowers.consume.chainTimerService;

  const timerBrand = await E(chainTimerService).getTimerBrand();

  t.log('timerBrand::', timerBrand);
  ...
  const instance = await E(zoe).startInstance(
    installation,
    { Fee },
    { ...makeTerms(), startTime: harden({ timerBrand, relValue: oneDay }) },
  ...
  return { instance, installation, timer: chainTimerService, publicFacet };
};
...
test('Airdrop ::: happy paths', async t => {
  ...
  const chainTimerService = await mockPowers.consume.chainTimerService;
  t.log({ chainTimerService });
  const { instance, publicFacet, timer } = await startLocalInstance(t, bundle, {
    zoe: zoeRef,
    issuers: { Fee: await E(zoeRef).getFeeIssuer() },
  });

  await E(chainTimerService).advanceBy(oneDay * 2n);
  t.log({
    mockPowers: Object.keys(mockPowers),
    chainStorage: Object.keys(chainStorage),
  });
  ...
  await t.throwsAsync(
    simulateClaim(t, zoeRef, instance.instance, feePurse, offerargs),
    {
...
//   // When the BLD staker governance proposal passes,
//   // the startup function gets called.
//   await startTribblesAirdrop(powers, {
//     options: {
//       customTerms: makeTerms(),
//       tribblesAirdrop: { bundleID: getBundleId(bundle) },
//     },
//   });
