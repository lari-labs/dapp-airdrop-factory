  ...
  const combinedHashes = handleCheckEven(hashes).reduce(
    (acc, _, index, arr) => {
      if (index % 2 === 0) {
        ...
        const hash = createSha256HexHash(hashPairConcatenated);
        acc.push(hash);
      }
      return acc;
    },
...
const createSha256HashObj = createHash(createSha256HexHash);
...
function* generate(hashes) {
  ...
  while (currentHashes.length > 1) {
    ensureEven(currentHashes);

    const combinedHashes = [];
    for (let i = 0; i < currentHashes.length; i += 2) {
      const hashesConcatenated = currentHashes[i] + currentHashes[i + 1];
      const hash = createSha256HexHash(hashesConcatenated);
      combinedHashes.push(hash);
    }

    yield combinedHashes;

    currentHashes = combinedHashes;
  }
}
...
const generateMerkleTree = (hashes = []) => {
  ...
  const tree = [hashes];
  for (const combinedHashes of generate(hashes)) {
...
export const merkleTreeAPI = {
  generateMerkleRoot(pks) {
    return generateMerkleRoot(pks.map(createSha256HexHash));
  },
  generateMerkleTree(pks) {
    return generateMerkleTree(pks.map(createSha256HexHash));
  },
  generateMerkleProof(pkHash, hashes) {
    return generateMerkleProof(
      createSha256HexHash(pkHash),
      hashes.map(createSha256HexHash),
    );
  },
...
const makeMerkleTreeAPI = (pubkeys, accounts) => ({
  ...
  get tree() {
    return merkleTreeAPI.generateMerkleTree(pubkeys);
  },
  constructProof(key) {
    return merkleTreeAPI.generateMerkleProof(key, this.pubkeys);
  },
